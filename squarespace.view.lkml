...
view: squarespace {
  derived_table: {
    sql: SELECT
          ...
          transaction_type,
          purchase_date_mon,
          to_timestamp((TO_CHAR(DATE_TRUNC('second', convert_timezone('US/Pacific', 'UTC',
              (date((DATE(date_dim.full_date ))) || ' ' || time_dim.time_of_day)::timestamp)), 'YYYY-MM-DD HH24:MI:SS')),'YYYY-MM-DD HH24:MI') AS invoice_ready_for_charge_datetime,
          ...
          sum(user_rate) as postage_price,
      ...
      where platform_name='Squarespace'
        and carrier_own_account_indicator='Managed 3rd Party Master Account'
        --and transaction_type in ('Purchase','Refund')
      and purchase_date>='2021-10-31'
  ...
  dimension_group: invoice_ready_for_charge_datetime {
    type: time
  ...
  measure: postage_price {
    type: sum
    sql: ${TABLE}.postage_price ;;
  }
  ...
  set: detail {
    fields: [
      ...
      transaction_type,
      purchase_date_mon,
      invoice_ready_for_charge_datetime_time,
      labels_count,
      label_markup,

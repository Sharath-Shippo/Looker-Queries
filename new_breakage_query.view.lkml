...
view: new_breakage_query {
  derived_table: {
    sql: --BREAKAGE QUERY BY CARRIER (UPS, USPS, FEDEX) FOR ACCOUNTING, UNUSED LABEL = FIRST SCAN DATE IS NOT NULL, REFUNDS PURCHASED OUT OF PERIOD
      ...
      --            END) no_labels_refunded_purchased_out_of_period,
          ...
          COUNT(DISTINCT
                CASE WHEN
                         (
                                     unused_labels_from_First_Event IN ('unused')
                                 AND
                                     query IN ('purchase_refund_in_period')
                             )
                         THEN tracking_number
                    END) no_unused_labels_purchased_from_FirstEvent,
          COUNT(DISTINCT
                CASE WHEN
                         (
                                     refund_status IN ('SUCCESS')
          ...
          COUNT(DISTINCT
                CASE WHEN
                         (
                                     unused_labels_from_Delivery_Date IN ('unused')
                                 AND
                                     query IN ('purchase_refund_in_period')
                             )
                         THEN tracking_number
                    END) no_unused_labels_purchased_from_DeliveryDate,
          COUNT(DISTINCT
                CASE WHEN
                         (
                                     refund_status IN ('SUCCESS')
          ...
          COUNT(DISTINCT
                CASE WHEN
                         (
                                     unused_labels_from_First_Scan IN ('unused')
                                 AND
                                     query IN ('purchase_refund_in_period')
                             )
                         THEN tracking_number
                    END) no_unused_labels_purchased_from_firstscan,
          COUNT(DISTINCT
                CASE WHEN
                         (
                                     refund_status IN ('SUCCESS')
      ...
      --            END) no_unused_labels_remaining,
          ...
          SUM(
                  CASE WHEN
                           ...
                           THEN COALESCE(shippo_cost,0)*-1
                      END) unused_label_cost_from_firstevent,
          SUM(
                  CASE WHEN
                           (
                                       refund_status IN ('SUCCESS')
                           ...
                           THEN COALESCE(ref_shippo_cost,0)*-1
                      END) unused_label_refund_cost_from_firstevent,
          --delivery Date--
          SUM(
                  CASE WHEN
                           (
                           ...
                           THEN COALESCE(shippo_cost,0)*-1
                      END) unused_label_cost_from_deliverydate,
          SUM(
                  CASE WHEN
                           (
                                       refund_status IN ('SUCCESS')
                           ...
                           THEN COALESCE(ref_shippo_cost,0)*-1
                      END) unused_label_refund_cost_from_deliverydate,
          --first scan Date--
          SUM(
                  CASE WHEN
                           (
                           ...
                           THEN COALESCE(shippo_cost,0)*-1
                      END) unused_label_cost_from_firstscan,
          SUM(
                  CASE WHEN
                           (
                                       refund_status IN ('SUCCESS')
          ...
          (
              SELECT DISTINCT
                  ...
                  CASE WHEN
                           ...
                           THEN 'unused'
                      END AS unused_labels_from_First_Event,
                  CASE WHEN
                           (
                                   tded.full_date IS NULL --Delivery Date
                                   OR
                                   tded.full_date = 0
                                   OR
                                   tded.full_date IN ('2035-12-31')
                               )
                           THEN 'unused'
                      END AS unused_labels_from_Delivery_Date,
                  CASE WHEN
                           (
                                   firscn.full_date IS NULL --First Scan Date
                                   OR
                                   firscn.full_date = 0
                                   OR
                                   firscn.full_date IN ('2035-12-31')
                               )
                           THEN 'unused'
              ...
              FROM prod.label_fact lf
                       ...
                       LEFT JOIN prod.date_dim firscn ON tf.first_scan_date_dim_id = firscn.date_dim_id --First Scan Date

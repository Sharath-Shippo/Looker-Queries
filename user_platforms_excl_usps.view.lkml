...
view: user_platforms_excl_usps {
  derived_table: {
    sql: SELECT
          'VIP REV SHARE' as Category,
          'User Platforms excl. USPS' as VIP_Revshare_Group,
          platform_name,
          purchase_date_mon,
          ...
          user_payment_method,
          invoice_id,
          --invoice_ready_for_charge_date_dim_id,
          carrier_name,
          carrier_service_level_name,
          transaction_type,
          sum(user_rev_share_usd) as user_rev_share_usd,
          sum(user_rate) as Postage_Price_USD,
          ...
          --sum("user_rate") total_postage_price_usd,
      ...
      where (platform_name ) <> 'Squarespace' AND (platform_name ) <> 'Shopify' and  (platform_name ) <> 'Shippo'
  ...
  measure: postage_price_usd {
    type: sum
    sql: ${TABLE}.postage_price_usd ;;
  }
